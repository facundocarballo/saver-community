...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    uint256 public timeSaverReward = 3 days; // 21 days;
    uint256 public timeStableCoinReward = 8 hours;
    ...
    address payable public projectsWallet =
        payable(0xC986D0E7249c78269efDE01466893a140C0b062c);
    address payable public developmentWallet =
        payable(0xE5eF8Bb53Ab058dd8d8cd51Cef935835D2b4993d);
    address public managementWallet = 0x9060723c22dE586c2fA5eFa07A7743F6f4a935f5;
    ...
    ERC20 BUSD = ERC20(0xd9C7C02dBF4451d89040cd2a576b615327ccF38b); // BUSD (18 decimals)

    // Saver Tokens
    ERC20 CDA = ERC20(0x14C11B212e25472cE25C018035a36117D0177D57);

    // ERC721
    ERC721 SinergyBronze;
    ERC721 SinergySilver;
    ERC721 SinergyGold;
    ...
    mapping(address => mapping(uint256 => bool)) public videoAnswerOf; // answerOf[wallet][dayNumber] => true
    ...
    function migrate(
        ...
        require(
            block.timestamp < timestampToCloseMigration,
        ...
        // CDA Migrate
        CDA.transfer(msg.sender, _CDABalance + _donationBalance);

        isRecover[msg.sender] = true;
    }

    function donateStableCoin(uint256 amount) public {
        require(
            BUSD.transferFrom(msg.sender, address(this), amount),
            "You have to approve the transaction first"
        );
        // 9% for development
        ...
        // 70% for rewards
        uint256 amountForReward = amount -
            amountForDevelopment -
            amountForProjects;
    ...
    function claim() public {
        ...
        require(
            !holderClaimed[msg.sender][rewardIDonClaim],
            "You already claim your reward."
        );

        require(
            rewardIDonClaim >= claimFrom[msg.sender],
            "You have to wait to the next reward to claim."
        );
        require(
            canReclaim(msg.sender),
            "You are not qualified to claim the reward"
        );
        require(
            qualifiedForSavings(msg.sender),
            "You have to increase your savings to be qualified"
        );
        ...
        require(
            stableCoinToClaim > 0,
            "You don't have any Stable Coin to claim."
        );

        require(
            amountRaised >= (amountClaimed + stableCoinToClaim),
            "The reward doesn't have the amount that you request, try it in the next reward."
        );

        require(
            donationBalance[msg.sender] >= (stableCoinToClaim / 3),
            "You can't receive this amount of USDC"
        );

        require(
            BUSD.transfer(msg.sender, stableCoinToClaim),
    ...
    function claimSaver() public {
        updateALL(msg.sender, 0);
        require(
            (_totalSupply + donationBalance[msg.sender]) < maxSupply,
            "The total supply of SAVER is already minted."
        );
        require(
        ...
        require(
            timestampToClaimSaver[msg.sender] < block.timestamp,
    ...
    function claimResourceForDevelopment() public {
        ...
        require(
            BUSD.transfer(developmentWallet, balanceCollectionForDevelopment),
            "Cannot pay StableCoin"
        );
    ...
    function claimResourcesForProjects() public {
        require(
            msg.sender == projectsWallet,
            "You are not qualified to call this function"
        );

        require(
            BUSD.transfer(projectsWallet, balanceCollectionForProjects),
            "Cannot pay StableCoin"
        );
    ...
    function convertCDAtoCDAToken(uint256 _amount) public {
        require(
            ...
            "You cannot convert this amount of BDD"
        );

        require(CDA.transfer(msg.sender, _amount), "Cannot pay CDA"); // Se multiplica por 10^12 ya que USDT tiene 6 decimales

        updateDonationBalanceAfterConvertBDDtoCDA(msg.sender, _amount);
    ...
    function answerVideo(
        ...
        videoAnswerOf[msg.sender][dayNumber] = first && second && third;
    }

    function changeSorted() public
    {
        require(msg.sender == communityWallet, "You are not qualified to call this function");
        sort = !sort;
    }
    ...
    function viewClaimStableCoin(address wallet) public view returns (uint256) {
        ...
        /*
            Si se da la condicion de que la cantidad a recibir es mayor que lo que tiene el bote
            y ademas lo que tiene el bote es mayor a mi potencial de ganancia, devolver solo el potencial de ganancia
        */

        if (amountRaised < (amountClaimed + amount))
            return amountRaised - amountClaimed;

        if (amount > (donationBalance[wallet] * 3))
            return donationBalance[wallet] * 3;
    ...
    function qualifiedForDonatePerDay(address wallet)
        public
        view
        returns (bool)
    {
        ...
        return (donatedByDayOf[wallet][dayNumber - 1] >= minDonateToQualify);
    }

    function qualifiedForVideo(address wallet) public view returns (bool) {
        if (dayNumber == 0) return videoAnswerOf[wallet][dayNumber];

        return videoAnswerOf[wallet][dayNumber - 1];
    }
    ...
    function getSavingsLimit(address wallet) public view returns (uint256) {
        ...
        return
            ((usdcRecord[wallet][id - 2] + daiRecord[wallet][id - 2]) *
                100369) / 100000;
    }

    function qualifiedForSavings(address wallet) public view returns (bool) {
        return ((getBalanceOfUSDC(wallet) + getBalanceOfDAI(wallet)) >
            getSavingsLimit(wallet));
    }

    function canReclaim(address wallet) public view returns (bool) {
        return (qualifiedForBDD(wallet) &&
            qualifiedForDAI(wallet) &&
            qualifiedForUSDC(wallet) &&
            qualifiedForSAVER(wallet) &&
            qualifiedForDonatePerDay(wallet) &&
            qualifiedForSavings(wallet) &&
            qualifiedForVideo(wallet)
            );
    }
    ...
    {
        require(msg.sender == managementWallet, "You are not qualified to call this function");
    ...
    function updateDonationBalanceAfterClaim(address wallet, uint256 amount)
        private
    {
        qualifiedDonationBalance -= donationBalance[wallet];
    ...
    function updateDonationBalanceAfterConvertBDDtoCDA(
        address wallet,
        uint256 amount
    ) private {
        if (isQualified[wallet]) {
    ...
    function updateQualifiedDonationBalancesAfterTransfer(address wallet)
        private
    {
    ...
    function updateQualifiedDonationBalancesBeforeClaim(address wallet)
        private
    {
        if (canReclaim(wallet) && !isQualified[wallet]) {
            qualifiedDonationBalance += donationBalance[wallet];
