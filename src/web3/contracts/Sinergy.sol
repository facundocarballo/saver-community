contract Sinergy is ERC721, ERC721URIStorage, Ownable, Router {
    ...
    // Amounts to transfer
    uint256 public PERCENT_REWARD = 83;
    uint256 public PERCENT_DIVIDE_REWARD = 1000;

    uint256 public PERCENT_STABLECOIN_VALUE_REWARD = 83;
    uint256 public PERCENT_DIVIDE_STABLECOIN_VALUE_REWARD = 1000;

    uint256 public PERCENT_STABLECOIN_CONSTANCY_REWARD = 83;
    uint256 public PERCENT_DIVIDE_STABLECOIN_CONSTANCY_REWARD = 1000;

    uint256 public PERCENT_STABLECOIN_CONFIDENCE_REWARD = 83;
    uint256 public PERCENT_DIVIDE_STABLECOIN_CONFIDENCE_REWARD = 1000;
    ...
    function HandleRecover(
        ...
        _RecoverNFT(tokenId, migration_contract);
    ...
    function NftWasQualified(uint256 tokenID) public view returns (bool) {
        return
            ...
            Qualification.qualified_history(
                ownerOf(tokenID),
                GetCycle() - 1
            );
    }
    ...
    function CreateGenesisNfts() public {
        ...
        for (uint256 i = 0; i < 9; i++) {
            ...
            _RecoverNFT(i, address(AprilSinergyBronzeContract));

            affiliate_rewards += AprilSinergyBronzeContract
                .get_nft_balance_to_claim(i);
    ...
    function CloseCycle() public {
        require(
            ...
            "Only the Clock can set the qualified nfts per cycle."
        );
    ...
    function GetAbleToPassiveReward() public view returns (uint256) {
        return
    ...
    function SetFavouriteNFT(address wallet, uint256 id) public {
        ...
        require(
            msg.sender == ownerOf(id) ||
                msg.sender == address(MigrationSinergy),
    ...
    function SetAbleValueRewardPercent(uint256 amount) public {
        require(
            IsOwner(msg.sender),
            "You are not qualified to modify prices and percentages."
        );
    ...
    function SetMinAmountFirstLevelConnections(uint256 amount) public {
        require(
            IsOwner(msg.sender),
            "You are not qualified to modify min amount first level connections."
        );
    ...
    function _TransferValueRewards() private {
        AbleValueReward.IncreaseReward(GetAbleToPassiveReward());
        Able.transfer(address(AbleValueReward), GetAbleToPassiveReward());

        StablecoinValueReward.IncreaseReward(GetStablecoinToPassiveReward());
        Stablecoin.transfer(
    ...
    function _TransferConstancyRewards() private {
        AbleConstancyReward.IncreaseReward(GetAbleToConstancyReward());
        Able.transfer(address(AbleConstancyReward), GetAbleToConstancyReward());

        StablecoinConstancyReward.IncreaseReward(
    ...
    function _TransferConfidenceRewards() private {
        AbleConfidenceReward.IncreaseReward(GetAbleToPassiveReward());
        Able.transfer(address(AbleConfidenceReward), GetAbleToPassiveReward());

        AbleConfidenceReward.IncreaseReward(GetStablecoinToPassiveReward());
        Stablecoin.transfer(
    ...
    function _Distribute(
        ...
        while (i < 9) {
            ...
            nft_affiliate_rewards_earned[_reference] += reward_amount;
            total_stablecoin_distributed += reward_amount;

            _TrnasferStablecoin(owner, tokenID, reward_amount);
    ...
    function _RecoverNFT(uint256 nftID, address contract_migration) private {
        SinergyMigration ContractMigration = SinergyMigration(
            contract_migration
        );
