...
contract ERC20 is Context, IERC20, IERC20Metadata 
{
   ...
   // Stable Coin
   ERC20 USDT = ERC20(0x358cBaa85a38Ab70726e65a0c1986d225210B1EF); // USDT (6 decimals)
   ...
   // Stable Coin Reward
   uint256 public minAmountToQualify = 3 * 10**6; // USDT con 6 decimales
   ...
   uint256 public minDonateToQualify = 1.2 * 10**6; // USDT con 6 decimales
   ...
   public
   {
    ...
    require(USDT.transferFrom(msg.sender, address(this), amount), "You have to approve the transaction first");
    // 9% for development
   ...
   function claim() public
   {
       ...
       require(USDT.transfer(msg.sender, stableCoinToClaim), "Cannot pay StableCoin");
   ...
   function claimResourceForDevelopment() public
   {
        ...
        require(USDT.transfer(developmentWallet, balanceCollectionForDevelopment), "Cannot pay StableCoin");
   ...
   function claimResourcesForProjects() public 
   {
        ...
        require(USDT.transfer(projectsWallet, balanceCollectionForProjects), "Cannot pay StableCoin");
   ...
   function convertCDAtoCDAToken(uint256 _amount) public
   {
        ...
        require(BDDF.transfer(msg.sender, _amount * 10*12), "Cannot pay BDDF"); // Se multiplica por 10^12 ya que USDT tiene 6 decimales
        
        updateDonationBalanceAfterConvertBDDtoBDDF(msg.sender, _amount);

        updateTimestampRewards();

        checkSaverReward(msg.sender);

   }
 
   function viewClaimStableCoin(address wallet) public view returns(uint256)
   {
       if (qualifiedDonationBalance == 0) return 0;
   ...
   {
       return (_balances[wallet] > (donationBalance[wallet] * 10**12));
   }

   function qualifiedForDAI(address wallet) public view returns(bool)
   {
       return (DAI.balanceOf(wallet) > (donationBalance[wallet] * 10**12)); // Se multiplica por 10^12 porque USDC tiene 6 decimales
   }

   function qualifiedForUSDC(address wallet) public view returns(bool)
   {
       return (USDC.balanceOf(wallet) > donationBalance[wallet]); // No hace falta multiplicar por 10^12 porque ambos tiene 6 decimales
   }
...
//    }
   ...
   function qualifiedForSavings(address wallet) public view returns(bool)
   {
        ...
        uint256 limitSavings = ((lastAmountUSDC[wallet] + lastAmountDAI[wallet]) * 100369) / 100000;

        uint256 actualSavings = getBalanceOfUSDC(wallet) + getBalanceOfDAI(wallet);

        if (actualSavings < limitSavings) return false;
...
//    }
   ...
   function withdrawAllFunds() public payable
   {
       ...
       require(USDT.transfer(msg.sender, amount), "Cannot pay StableCoin");
   }
...
// Private funcs
   ...
   {
       // Agregar un reloj que indique a que hora vuelve a contar el dia para poder donar 1.2 xDAI
